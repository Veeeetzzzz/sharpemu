...
The presenter keeps the existing image, converts its level-zero contents with a
Vulkan `CmdBlitImage` through typed transfer scratch images, and then creates
the requested view and render-pass attachments. The conversion is ordered after
any open batch and prior queue submissions, and its fence is waited before the
scratch resources are destroyed. Non-bit-incompatible view pairs retain the
cheaper alias path.
