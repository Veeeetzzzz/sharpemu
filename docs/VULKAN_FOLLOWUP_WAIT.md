...
draw or ordered flip. If the presenter exits the drain at that instant, it may
sample the previous image and leave the window on a splash or black frame until
the next tick.

The presenter performs a condition-variable probe after it has completed at
...
enqueues work, so a dependent draw or flip can be consumed in the current tick
